...
def get_data():
    ...
    #每个经理building_rt的均值
    all = merge_nunique(all, ["manager_id"], "building_rt", "manager_pay")

    #根据经纬度类别构造特征

    #区域内有多少不同经理竞争，即为经理数
    print "区域内有多少不同经理竞争"
    all = merge_nunique(all, ["jwd_class"], "manager_id", "manager_num_jwd")

    #一个经理经营多少个区域
    print "一个经理经营多少个区域"
    all = merge_nunique(all, ["manager_id"], "jwd_class", "manager_jwd_class")

    #该区域内的均价
    print "区域内的均价"
    all = merge_median(all, ["jwd_class"], "price", "price_mean_jwd")

    #该区域内的building数
    print "该区域内的building数"
    all = merge_nunique(all, ["jwd_class"], "building_id", "building_num_jwd")


    #每个manager的平均放照片多少，描述字的多少，反映经理工作的仔细程度
    print "每个manager的平均放照片多少"
    all = merge_mean(all, ["manager_id"], "photo_num", "manager_photo")

    print "每个manager的平均描述字多少"
    all = merge_mean(all, ["manager_id"], "num_description_words", "manager_desc")

    print "每个manager的平均描述feature个数"
    all = merge_mean(all, ["manager_id"], "feature_num", "manager_feature")

    #各种房型的均价，然后减去当前房子的均价就是地理位置带来的影响
    all = merge_median(all, ["bathrooms","bedrooms"], "price", "fangxing_mean")
    ...
    # 在某经纬度范围内各种房型的均值
    all = merge_median(all, ["jwd_class","bathrooms","bedrooms"], "price", "type_jwd_price_mean")
    ...
    #该manager在该地区出价的比的平均
    all = merge_mean(all, ["manager_id"], "type_jwd_price_mean_rt", "manager_pay_jwd")

    #该building在该地区出价的比的平均
    all = merge_mean(all, ["building_id"], "type_jwd_building_mean_rt", "building_pay_jwd")

    #该jwd在该市出价的比的平均
    all = merge_mean(all, ["jwd_class"], "fangxing_mean_rt_jwd", "jwd_pay_all")

    #该经理拥有的房子比周边贵还是便宜
    all = merge_mean(all, ["manager_id"], "building_pay_jwd", "manager_own_ud")

    #该经理拥有的房子地区比该市贵还是便宜
    all = merge_mean(all, ["manager_id"], "jwd_pay_all", "manager_own_ud_all")
    ...
    all = merge_nunique(all, ["manager_id"], "all_hours", "manager_hours")
    ...
    all = merge_sum(all, ["manager_id"], "price", "manager_price_sum")

    #经理总共发出多少个卧室的listing
    all = merge_sum(all, ["manager_id"], "bedrooms", "manager_bedrooms_sum")

    #经理总共挣了多少钱
    all = merge_sum(all, ["manager_id"], "building_dif", "earn_all")
    ...
    #用低于该价格多少来表示经理开价程度
    all = merge_mean(all, ["manager_id"], "jwd_type_rt", "manager_pay_jwd_type_rt")
    ...
    #经理的活动范围距离市区多远
    all = merge_mean(all, ["manager_id"], "distance", "manager_distance")

    # 经理发帖时间稳定性
    all = merge_var(all, ["manager_id"], "created_hour", "manager_post_hour_var")

    # 经理发帖时间集中在哪里
    all = merge_mean(all, ["manager_id"], "created_hour", "manager_post_hour_mean")
    ...
    all = merge_mean(all, ["manager_id"], "price_bath_bed", "manager_price_bath_bed_mean")
    ...
    #manager的经纬度中位数
    all = merge_median(all, ["manager_id"], "longitude", "manager_longitude_median")
    all = merge_median(all, ["manager_id"], "latitude", "manager_latitude_median")
    ...
    #该区域内的listing数
    all = merge_count(all, ["jwd_class"], "listing_id", "listing_num_jwd")
    ...
    all = merge_median(all, ["time"], "price", "price_today")

    all = merge_median(all, ["created_month"], "price", "price_today")
