...
def et(x_train, y_train, x_valid):
    extratree = ExtraTreesClassifier(n_estimators=1200, max_depth=22, max_features="auto", n_jobs=-1, random_state=2017,verbose=1)
...
if __name__=="__main__":
    ...
    '''
    #onevsrest转换high和medium一起
    y_train[y_train==1]=0
    y_train[y_train==2]=1

    #onevsrest转换low和medium一起
    #y_train[y_train==2]=1
    ...
    #clf_list = [xgb_reg,lgb_reg,nn_reg,lgb,xgb,lr,rf,et,gb,nn,knn]

    #clf_list = [gb_reg,rf_reg,et_reg]
    clf_list = [lgb,nn,ada,lgb_reg,nn_reg,et,rf]
    #
    #
    #
    column_list = []
    ...
    for clf in clf_list:
        ...
        for ind in range(ind_num):
            column_list.append("ori_ovrhm_%s_%s" % (clf_name, ind))
    ...
    train.to_csv("ori_ovrhm_train_stacking.csv", index=None)
    test.to_csv("ori_ovrhm_test_stacking.csv", index=None)
